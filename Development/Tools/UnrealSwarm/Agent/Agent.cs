...
namespace Agent
{
	...
	public partial class Agent : MarshalByRefObject, IAgentInternalInterface, IAgentInterface
	{
		...
		public bool Init( Int32 NewAgentProcessID )
		{
			...
			X509Certificate ExecutableCertificate = null;
			try
			{
				ExecutableCertificate = X509Certificate.CreateFromSignedFile( AgentFileName );
			}
			catch( Exception )
			{
				// An unsigned executable has no certificate to extract.
			}

			if( ExecutableCertificate == null )
			{
				// Local source builds are intentionally unsigned. This exception is
				// safe only in standalone mode, where Swarm neither advertises itself
				// to a Coordinator nor accepts or distributes remote work.
				if( !ApplicationDeployment.IsNetworkDeployed &&
					AgentApplication.Options.EnableStandaloneMode )
				{
					Log( EVerbosityLevel.Informative, Color.DarkGreen, " ...... unsigned local-source executable accepted for standalone-only execution" );
				}
				else
				{
					Log( EVerbosityLevel.Critical, Color.Red, " ......... unsigned executable rejected: distributed or published Swarm requires a validated deployment certificate" );
					return false;
				}
			}
			else
			{
				Log( EVerbosityLevel.Verbose, Color.DarkGreen, " ...... checking deployment certificate" );
				...
				if( Certificate == null )
				{
					Log( EVerbosityLevel.Critical, Color.Red, " ......... deployment certificate validation failed" );
					return false;
				}
				Log( EVerbosityLevel.Informative, Color.DarkGreen, " ......... deployment certificate validated" );
			}
			...
			// Initialize either the configured coordinator or the explicit local-only policy.
			InitCoordinator();

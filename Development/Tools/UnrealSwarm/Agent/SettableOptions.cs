...
namespace Agent
{
	public enum EOptionsVersion
	{
		...
		LocalSourceBuildDefaults			= 15,

		Current							= LocalSourceBuildDefaults
	};
    ...
    public class SettableOptions
    {
		...
		[XmlAttribute]
		private bool LocalEnableStandaloneMode = true;
		...
		[XmlTextAttribute]
		private string LocalAllowedRemoteAgentNames = "";
		...
		[XmlTextAttribute]
		private string LocalAllowedRemoteAgentGroup = "Default";
		...
		[XmlTextAttribute]
		private string LocalCoordinatorRemotingHost = "127.0.0.1";
		...
		private void SetDefaults()
		{
			...
			bool IsPublishedDeployment = ApplicationDeployment.IsNetworkDeployed;
			EnableStandaloneMode = !IsPublishedDeployment;
			AllowedRemoteAgentNames = ( IsPublishedDeployment ? "RENDER*" : "" );
			AllowedRemoteAgentGroup = ( IsPublishedDeployment ? "DefaultDeployed" : "Default" );
			AgentGroupName = ( IsPublishedDeployment ? "DefaultDeployed" : "Default" );
			CoordinatorRemotingHost = ( IsPublishedDeployment ? "RENDER-01" : "127.0.0.1" );
		...
		{
			if( OptionsVersion < (int)EOptionsVersion.AddPerformanceMonitoringDisable || SerialisableColours.Count == 0 )
			{
				SetDefaults();
			}
			else if( OptionsVersion < (int)EOptionsVersion.LocalSourceBuildDefaults )
			{
				// Version 14 carried Epic's internal farm defaults. Migrate only
				// those values for a local source build and preserve the user's
				// cache size, cache location, colours, and other preferences.
				if( !ApplicationDeployment.IsNetworkDeployed )
				{
					EnableStandaloneMode = true;
					if( String.Equals( AllowedRemoteAgentNames, "RENDER*", StringComparison.OrdinalIgnoreCase ) )
					{
						AllowedRemoteAgentNames = "";
					}
					if( String.Equals( AllowedRemoteAgentGroup, "DefaultDeployed", StringComparison.OrdinalIgnoreCase ) )
					{
						AllowedRemoteAgentGroup = "Default";
					}
					if( String.Equals( AgentGroupName, "DefaultDeployed", StringComparison.OrdinalIgnoreCase ) )
					{
						AgentGroupName = "Default";
					}
					if( String.Equals( CoordinatorRemotingHost, "RENDER-01", StringComparison.OrdinalIgnoreCase ) )
					{
						CoordinatorRemotingHost = "127.0.0.1";
					}
				}
				OptionsVersion = (int)EOptionsVersion.Current;
			}

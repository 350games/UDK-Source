...
{
	DWORD SampleRate = XAudio2Device->GetOutputSampleRate();
	...
	const DWORD NumChannels = UXAudio2Device::NumSpeakers;
	UXAudio2Device::GetOutputMatrix( XAudio2Device->GetOutputChannelMask(), NumChannels );
...
{
#if WITH_MODERN_XAUDIO2
	// The custom CXAPOParametersBase implementation targets the legacy XAPO
	// ABI. Keep ordinary XAudio2/EQ/reverb active and omit radio distortion.
	return FALSE;
#else
	// Grab the sample rate, which is needed to configure the radio distortion effect settings.
	const DWORD SampleRate = XAudio2Device->GetOutputSampleRate();

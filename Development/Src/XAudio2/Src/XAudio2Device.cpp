...
#if WITH_MODERN_XAUDIO2
UINT32 UXAudio2Device::OutputSampleRate					= 0;
DWORD UXAudio2Device::OutputChannelMask					= 0;
#else
...
UBOOL UXAudio2Device::InitHardware( void )
{
	if( XAudio2 == NULL )
	{
...
#if WITH_MODERN_XAUDIO2
		// XAudio2 2.9 no longer exposes device enumeration.  Create the default
		// mastering voice, then query the effective output format from it.
		if( XAudio2->CreateMasteringVoice( &MasteringVoice, XAUDIO2_DEFAULT_CHANNELS, XAUDIO2FX_REVERB_MAX_FRAMERATE, 0, NULL, NULL, AudioCategory_GameEffects ) != S_OK )
		{
		...
		if( MasteringVoice->GetChannelMask( &OutputChannelMask ) != S_OK )
		{
			debugf( NAME_Init, TEXT( "Failed to query the XAudio2 channel mask" ) );
			MasteringVoice->DestroyVoice();
			MasteringVoice = NULL;
			XAudio2 = NULL;
			return( FALSE );
		...
		NumSpeakers = VoiceDetails.InputChannels;
		SampleRate = VoiceDetails.InputSampleRate;
		OutputSampleRate = SampleRate;
		debugf( NAME_Init, TEXT( "XAudio2 using the default output: %d channels at %g kHz (channel mask 0x%x)" ), NumSpeakers, ( FLOAT )SampleRate / 1000.0f, OutputChannelMask );

		if( !GetOutputMatrix( OutputChannelMask, NumSpeakers ) )
		{

...
namespace UnrealBuildTool
{
    ...
    abstract class Utils
    {
		...
		private static void CompileCPPProject( CPPEnvironment CompileEnvironment, UE3ProjectDesc ProjectDesc, ref LinkEnvironment LinkEnvironment )
		{
		    ...
		    ProjectCPPEnvironment.AdditionalArguments += ProjectDesc.AdditionalCompilerArguments;
			...
			if (CompileEnvironment.ShouldUsePCHs() && !ProjectDesc.bDisablePCH)
		    {
                ...
                foreach (KeyValuePair<string, PrecompileHeaderEnvironment> PCHUsage in UsageMapPCH)
                {
                        ...
                        PCHEnvironment.CompileEnvPCH != null)
                    {
                        ...
                        if (BuildConfiguration.bUseUnityBuild && !ProjectDesc.bDisableUnity)
                        {
		...
		{
			SetEnvironmentVariablesFromBatchFile( BatchFileName, "" );
		}

		/**
		 * Sets the environment variables from the passed in batch file, passing arguments to it.
		 *
		 * @param	BatchFileName	Name of the batch file to parse
		 * @param	BatchFileArguments	Arguments passed to the batch file
		 */
		public static void SetEnvironmentVariablesFromBatchFile(string BatchFileName, string BatchFileArguments)
		{
			if( File.Exists( BatchFileName ) )
			{
...

				{
					...
					EnvReaderBatchFileContent.Add( String.Format( "call \"{0}\" {1}", BatchFileName, BatchFileArguments ) );

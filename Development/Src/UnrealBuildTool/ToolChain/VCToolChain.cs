...
namespace UnrealBuildTool
{
	class VCToolChain
	{
		/** Whether the active compiler was discovered through a modern Visual Studio installation. */
		static bool bUsingModernVCToolchain = false;

		/** Directory that contains the active modern MSVC toolset. */
		static string ModernVCToolsDir = "";

		/** Versioned Windows SDK directory name used by modern resource compilers. */
		static string ModernWindowsSDKVersion = "";
		...
		static string GetCLArguments_Global(CPPEnvironment CompileEnvironment)
		{
			...
			if( bUsingModernVCToolchain )
			{
				// These legacy source files intentionally use local/member shadowing that
				// predates the diagnostics added by current MSVC releases.
				Result += " /wd4456 /wd4457 /wd4458 /wd4459";
			}
			...
			// Keep the recovered target self-contained without changing the runtime
			// selection for the original UE3 targets.
			bool bUseStaticRuntimeLibrary = CompileEnvironment.Definitions.Contains( "WITH_LOCAL_UTGAME_STATIC_CRT=1" );
		...
		{
			// Initialize before materializing include arguments.  The legacy DirectX9
			// include bundle also contains an obsolete rpcsal.h, so the current Windows
			// SDK shared headers must precede it when using a modern toolchain.
			InitializeEnvironmentVariables( CompileEnvironment.TargetPlatform );
			if( bUsingModernVCToolchain )
			{
				if( CompileEnvironment.Definitions.Contains( "WITH_MODERN_XAUDIO2=1" )
					|| CompileEnvironment.Definitions.Contains( "WITH_LOCAL_UTGAME_D3D9_SHADER_COMPILER=1" ) )
				{
					string WindowsSDKUMDirectory = Path.Combine( WindowsSDKDir, "Include", ModernWindowsSDKVersion.TrimEnd( '\\', '/' ), "um" );
					if( Directory.Exists( WindowsSDKUMDirectory ) && !CompileEnvironment.SystemIncludePaths.Contains( WindowsSDKUMDirectory ) )
					{
						// Prefer the current XAudio2/X3DAudio and D3DCompiler headers while
						// retaining the bundled D3DX9 headers later in the include search path.
						CompileEnvironment.SystemIncludePaths.Insert( 0, WindowsSDKUMDirectory );
					}
				}

				string WindowsSDKSharedDirectory = Path.Combine( WindowsSDKDir, "Include", ModernWindowsSDKVersion.TrimEnd( '\\', '/' ), "shared" );
				if( Directory.Exists( WindowsSDKSharedDirectory ) && !CompileEnvironment.SystemIncludePaths.Contains( WindowsSDKSharedDirectory ) )
				{
					CompileEnvironment.SystemIncludePaths.Insert( 0, WindowsSDKSharedDirectory );
				}
			}
		...
		public static FileItem LinkFiles(LinkEnvironment LinkEnvironment)
		{
			...
			// A recovered local target can request the modern XInput import library
			// without hard-coding a Windows SDK installation path in its target file.
			// Resolve the marker only after GetVCToolPath has initialized vcvarsall.
			int WindowsSDKXInputIndex = LinkEnvironment.AdditionalLibraries.FindIndex(
				Library => string.Equals( Library, "WindowsSDKXInput.lib", StringComparison.OrdinalIgnoreCase ) );
			if( WindowsSDKXInputIndex >= 0 )
			{
				if( !bUsingModernVCToolchain )
				{
					throw new BuildException( "WindowsSDKXInput.lib requires the modern Windows SDK toolchain." );
				}

				string SDKArchitecture = LinkEnvironment.TargetPlatform == CPPTargetPlatform.Win64 ? "x64" : "x86";
				string WindowsSDKXInputPath = Path.Combine(
					WindowsSDKDir,
					"Lib",
					ModernWindowsSDKVersion.TrimEnd( '\\', '/' ),
					"um",
					SDKArchitecture,
					"xinput.lib" );
				if( !File.Exists( WindowsSDKXInputPath ) )
				{
					throw new BuildException( "Could not locate the Windows SDK XInput import library: {0}", WindowsSDKXInputPath );
				}

				LinkEnvironment.AdditionalLibraries[WindowsSDKXInputIndex] = WindowsSDKXInputPath;
			}
...
//			BuildProjectAction.CommandArguments += " /nologo /verbosity:minimal";
		...
		{	
			...
			if( bUsingModernVCToolchain )
			{
				string TargetArchitecture = Platform == CPPTargetPlatform.Win64 ? "x64" : "x86";

				if( ToolName.ToUpperInvariant() == "RC" )
				{
					VCToolPath = Path.Combine(
						WindowsSDKDir,
						"bin",
						ModernWindowsSDKVersion.TrimEnd( '\\' ),
						TargetArchitecture,
				...
				{
					string HostArchitecture = bSupports64bitExecutables ? "Hostx64" : "Hostx86";
					VCToolPath = Path.Combine( ModernVCToolsDir, "bin", HostArchitecture, TargetArchitecture, ToolName + ".exe" );
				}

				if( !File.Exists( VCToolPath ) )
				{
					throw new BuildException( "Could not locate the requested Visual C++ tool: {0}", VCToolPath );
				}
		...
		static string GetDotNetFrameworkToolPath(CPPTargetPlatform Platform, string ToolName)
		{
			...
			if( bUsingModernVCToolchain )
			{
				string VisualStudioInstallDir = Environment.GetEnvironmentVariable( "VSINSTALLDIR" );
				if( !string.IsNullOrEmpty( VisualStudioInstallDir ) )
				{
					string ModernMSBuildPath = Path.Combine( VisualStudioInstallDir, "MSBuild", "Current", "Bin", ToolName + ".exe" );
					if( File.Exists( ModernMSBuildPath ) )
					{
						return ModernMSBuildPath;
					}
				}
			}
		...
		/** Finds a Visual Studio installation with the C++ toolchain when VS2010 is not available. */
		static string FindModernVisualStudioInstallPath()
		{
			string ExistingInstallPath = Environment.GetEnvironmentVariable( "VSINSTALLDIR" );
			if( !string.IsNullOrEmpty( ExistingInstallPath )
				&& File.Exists( Path.Combine( ExistingInstallPath, "VC", "Auxiliary", "Build", "vcvarsall.bat" ) ) )
			{
				return ExistingInstallPath;
			}
			...
			try
			{
				...
				StartInfo.Arguments = "-latest -products * -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath";
				StartInfo.UseShellExecute = false;
				StartInfo.RedirectStandardOutput = true;
				StartInfo.CreateNoWindow = true;

				using( Process QueryProcess = Process.Start( StartInfo ) )
				{
					string InstallPath = QueryProcess.StandardOutput.ReadToEnd().Trim();
					QueryProcess.WaitForExit();
					if( QueryProcess.ExitCode == 0 && Directory.Exists( InstallPath ) )
					{
						return InstallPath;
					}
				}
			...
			{
				// Preserve the usual toolchain error below when discovery is unavailable.
			}
		...
		static void InitializeEnvironmentVariables( CPPTargetPlatform Platform )
		{
			if( !bAreEnvironmentVariablesAlreadyInitialized )
			{
				...
				// Prefer the original VS2010 layout when it is available.
				string BaseVSToolPath = Environment.GetEnvironmentVariable("VS100COMNTOOLS");
				if (string.IsNullOrEmpty(BaseVSToolPath))
				{
					string ModernVisualStudioInstallPath = FindModernVisualStudioInstallPath();
					if( string.IsNullOrEmpty( ModernVisualStudioInstallPath ) )
					{
						throw new BuildException( "Could not locate Visual Studio 2010 or a modern Visual Studio C++ toolchain." );
					}

					VCVarsBatchFile = Path.Combine( ModernVisualStudioInstallPath, "VC", "Auxiliary", "Build", "vcvarsall.bat" );
					if( !File.Exists( VCVarsBatchFile ) )
					{
						throw new BuildException( "Could not locate the Visual Studio environment script: {0}", VCVarsBatchFile );
					}

					Utils.SetEnvironmentVariablesFromBatchFile( VCVarsBatchFile, Platform == CPPTargetPlatform.Win64 ? "x64" : "x86" );

					ModernVCToolsDir = Environment.GetEnvironmentVariable( "VCToolsInstallDir" );
					WindowsSDKDir = Environment.GetEnvironmentVariable( "WindowsSdkDir" );
					ModernWindowsSDKVersion = Environment.GetEnvironmentVariable( "WindowsSDKVersion" );
					if( string.IsNullOrEmpty( ModernVCToolsDir )
						|| string.IsNullOrEmpty( WindowsSDKDir )
						|| string.IsNullOrEmpty( ModernWindowsSDKVersion ) )
					{
						throw new BuildException( "The modern Visual Studio environment did not provide MSVC and Windows SDK paths." );
					}

					bSupports64bitExecutables = Environment.Is64BitOperatingSystem;
					bUsingModernVCToolchain = true;

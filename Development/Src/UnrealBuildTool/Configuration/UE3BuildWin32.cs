...
namespace UnrealBuildTool
{
	partial class UE3BuildTarget
	{
        ...
        {            
			...
			bool bUseModernXAudio2 = GlobalCPPEnvironment.Definitions.Contains("WITH_MODERN_XAUDIO2=1");
			// XAudio2 2.9 is supplied by Windows 10 and later.  Keep the recovered
			// target's SDK declarations honest instead of advertising the older Win8
			// minimum that was used by previous XAudio2 releases.
			GlobalCPPEnvironment.Definitions.Add( bUseModernXAudio2 ? "_WIN32_WINNT=0x0A00" : "_WIN32_WINNT=0x0502" );
			GlobalCPPEnvironment.Definitions.Add( bUseModernXAudio2 ? "WINVER=0x0A00" : "WINVER=0x0502" );
			bool bCompileD3D11RHI = !GlobalCPPEnvironment.Definitions.Contains("WITH_D3D11_RHI=0");
			bool bCompileOpenGLRHI = !GlobalCPPEnvironment.Definitions.Contains("WITH_OPENGL_RHI=0");
			...
			if (Platform == UnrealTargetPlatform.Win64 && !GlobalCPPEnvironment.Definitions.Contains("WITH_LOCAL_UTGAME_NO_TBB=1"))
			{
			...
			// Compile and link with DirectShow when AVI capture has its SDK available.
            if (!GlobalCPPEnvironment.Definitions.Contains("WITH_AVIWRITER=0"))
            {
			...
			string ZlibIncludeDirectory = "../External/zlib/inc";
			string ZlibLibraryDirectory = "../External/zlib/Lib";
			bool bUsingBundledZlibFallback = false;
			if( !File.Exists( Path.Combine( ZlibIncludeDirectory, "zlib.h" ) ) )
			{
				// The dedicated zlib SDK folder was stripped, but this snapshot retains
				// matching zlib source with wx.
				ZlibIncludeDirectory = "../External/wxWindows_2.4.0/src/zlib";
				ZlibLibraryDirectory = "../External/wxWindows_2.4.0/lib";
				bUsingBundledZlibFallback = true;
			}
			GlobalCPPEnvironment.SystemIncludePaths.Add( ZlibIncludeDirectory );
			FinalLinkEnvironment.LibraryPaths.Add( ZlibLibraryDirectory );
			// UE3BuildLocalUTGame supplies zlib.Local.vcxproj, which provides the
			// fallback zlib objects for either Windows architecture.
			bool bUseSourceBuiltZlib = bUsingBundledZlibFallback && GlobalCPPEnvironment.Definitions.Contains( "WITH_LOCAL_UTGAME=1" );
			if( !bUseSourceBuiltZlib && Platform == UnrealTargetPlatform.Win64 )
			{
				if( !File.Exists( Path.Combine( ZlibLibraryDirectory, "zlib_64.lib" ) ) )
				{
					throw new BuildException( "Win64 requires the missing External/zlib zlib_64.lib; the bundled wx zlib fallback is Win32-only." );
				}
				FinalLinkEnvironment.AdditionalLibraries.Add( "zlib_64.lib" );
			}
			else if( !bUseSourceBuiltZlib )
			{
			...
			string DirectXSDKDirectory = Environment.GetEnvironmentVariable( "DXSDK_DIR" );
			if( String.IsNullOrEmpty( DirectXSDKDirectory ) || !Directory.Exists( DirectXSDKDirectory ) )
			{
				// The snapshot carries the legacy D3D9 headers and libraries needed by
				// the lean Win32 target, so it does not need a machine-wide DXSDK install.
				DirectXSDKDirectory = "../External/DirectX9";
			}

			GlobalCPPEnvironment.SystemIncludePaths.Add( Path.Combine( DirectXSDKDirectory, "Include" ) );
			string DirectXLibraryDirectory = Path.Combine( DirectXSDKDirectory, "Lib", Platform == UnrealTargetPlatform.Win64 ? "x64" : "x86" );
			if( Platform != UnrealTargetPlatform.Win64 && !Directory.Exists( DirectXLibraryDirectory ) )
			{
				// Bundled 32-bit libraries live directly under Lib, unlike the old SDK.
				DirectXLibraryDirectory = Path.Combine( DirectXSDKDirectory, "Lib" );
			}
			FinalLinkEnvironment.LibraryPaths.Add( DirectXLibraryDirectory );
			...
			if (!IsBuildingDedicatedServer() && bCompileOpenGLRHI)
			{
...
/*                if (UE3BuildConfiguration.bForceScaleformRelease && Configuration == UnrealTargetConfiguration.Debug)
			...
			if( Platform == UnrealTargetPlatform.Win32 && !GlobalCPPEnvironment.Definitions.Contains( "WITH_OPEN_AUTOMATE=0" ) )
			{

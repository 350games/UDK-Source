...
namespace UnrealBuildTool
{
	...
	class UE3ProjectDesc
	{
		...
		/** Keep legacy third-party source files in their original translation units. */
		public bool bDisableUnity = false;

		/** Do not infer an engine-style PCH for standalone third-party sources. */
		public bool bDisablePCH = false;

		/** Optional compiler switches that apply only to this project. */
	...
	partial class UE3BuildTarget
	{
		...
		/** Optional base name for a target whose executable name differs from its game/module name. */
		string DefaultOutputBaseName = null;
		...
		void ParseArguments(string[] SourceArguments)
		{
			...
			for (int ArgumentIndex = 0; ArgumentIndex < Arguments.Count; ArgumentIndex++)
			{
				switch (Arguments[ArgumentIndex].ToUpperInvariant())
				{
					...
					case "EXOGAME":
						...
						break;
					case "UTGAME":
						Game = new UE3BuildLocalUTGame( LocalUTGameTargetType.Runtime );
						break;
					case "UTGAMESCRIPTCOMPILER":
						Game = new UE3BuildLocalUTGame( LocalUTGameTargetType.ScriptCompiler );
						break;
					case "UDK":
						Game = new UE3BuildLocalUTGame( LocalUTGameTargetType.Editor );
						DefaultOutputBaseName = "UDK";
						break;
					case "UDKGAME":
			...
			if( OutputPath == null )
			{
				...
				string ExecutableBaseName = DefaultOutputBaseName;
				if( String.IsNullOrEmpty( ExecutableBaseName ) )
				{
					ExecutableBaseName = GetGameName() + Game.GetSubPlatform();
				}
				// e.g. UDKGameServer-Win32-Shipping.exe
				string ExecutableName = ExecutableBaseName + "-" + Platform.ToString() + "-" + Configuration.ToString() + GetOutputExtension( Platform );

				// Example: Binaries\\Win32\\ExampleGame.exe
				if( Configuration == UnrealTargetConfiguration.Release )
				{
					ExecutableName = ExecutableBaseName + GetOutputExtension( Platform );
				}

...
public:
	...
	WxPreferencesMenu()
	{
		// Keep the recovered settings surfaces together at the top of the
		// conventional Preferences menu.  This makes both the editor-wide and
		// project-wide settings discoverable without adding a non-standard
		// top-level menu to the UDK layout.
		Append( IDM_MainMenu_OpenEditorPreferences,
			TEXT( "Editor Settings..." ),
			TEXT( "Configure editor-wide preferences, viewport behavior, saving, and splash branding." ) );
		Append( IDM_MainMenu_OpenProjectSettings,
			TEXT( "Project Settings..." ),
			TEXT( "Configure project rendering, plugins, startup map, and diagnostics." ) );
		AppendSeparator();
...
BEGIN_EVENT_TABLE( WxEditorFrame, wxFrame )
	...
	EVT_MENU( IDM_MainMenu_OpenEditorPreferences, WxEditorFrame::MenuOpenEditorPreferences )
	EVT_MENU( IDM_MainMenu_OpenProjectSettings, WxEditorFrame::MenuOpenProjectSettings )
...
FString WxEditorFrame::GetLocalizedCaption(FString levelName)
{
	...
	#if UDK
		FString AppName = FString::Printf( LocalizeSecure( LocalizeUnrealEd( "UDKTitle_RHI_F" ), *PlatformBitsString, *RHIName ) );
	#else
		const FString GameName = GConfig->GetStr(TEXT("URL"), TEXT("GameName"), GEngineIni);
		FString AppName = FString::Printf( LocalizeSecure( LocalizeUnrealEd( "UnrealEdTitle_RHI_F" ), *GameName, *PlatformBitsString, *RHIName ) );
	#endif

	if (AppName.Len() == 0)
	{
		AppName = TEXT("UnrealEd");
	}
	...
	return FString::Printf(
		LocalizeSecure(LocalizeUnrealEd("UnrealEdCaption_F"), *levelName, *AppName));
}
...
/** Launch one of the settings panels supplied with this recovered build. */
static void LaunchUE3WorkbenchSettings( const TCHAR* SettingsMode )
{
	const FFilename WorkbenchExecutable( FString( appBaseDir() ) + FString( TEXT( "..\\UE3Workbench.exe" ) ) );
	if( !WorkbenchExecutable.FileExists() )
	{
		appMsgf(
			AMT_OK,
			TEXT( "The UE3 Workbench settings tool could not be found.\\n\\n%s\\n\\nRun the local build to restore it." ),
			*WorkbenchExecutable );
		return;
	}

	FString Error;
	appLaunchURL( *WorkbenchExecutable, SettingsMode, &Error );
	if( Error.Len() > 0 )
	{
		appMsgf( AMT_OK, TEXT( "Unable to launch the UE3 Workbench settings tool.\\n\\n%s" ), *Error );
	}
}

/** Opens the editor-wide settings panel from Preferences. */
void WxEditorFrame::MenuOpenEditorPreferences( wxCommandEvent& In )
{
	LaunchUE3WorkbenchSettings( TEXT( "editor-settings" ) );
}

/** Opens the project-wide settings panel from Preferences. */
void WxEditorFrame::MenuOpenProjectSettings( wxCommandEvent& In )
{
	LaunchUE3WorkbenchSettings( TEXT( "project-settings" ) );
}

...
/**
 * Loads a map template into a new untitled package.
 *
 * @return TRUE when the template was found (including when the user cancels
 *         the dirty-package save prompt), FALSE when no matching file exists.
 */
static UBOOL LoadMapTemplate(const FString& TemplateName)
{
	for (INT FolderIndex = 0; FolderIndex < GEditor->TemplateMapFolders.Num(); ++FolderIndex)
	{
		const FString& Folder = GEditor->TemplateMapFolders(FolderIndex);
		const FString TemplatePath = FString::Printf(TEXT("%s\\%s.umap"), *Folder, *TemplateName);
		if (GFileManager->FileSize(*TemplatePath) > 0)
		{
			// Loading with LoadAsTemplate set creates an untitled package, so the
			// source template can never be overwritten by an ordinary Save.
	...
	GWarn->Logf(NAME_Error, TEXT("Couldn't find template map file %s.umap in any configured TemplateMapFolders entry"), *TemplateName);
	appMsgf(AMT_OK, TEXT("Could not find the selected level template '%s.umap'. The current level was left unchanged."), *TemplateName);
	return FALSE;
}
...
void FEditorFileUtils::NewMapInteractive()
{
	...
	UBOOL bCreateMap = FALSE;

#if WITH_MANAGED_CODE
	// The managed screen returns an empty name for its explicit Blank Map item.
	bCreateMap = FNewMapScreen::DisplayNewMapScreen(Templates, TemplateName);
#else
	// The source-build target intentionally does not require the retired CLR
	// editor bridge. Keep the complete template workflow available through a
	// native wxWidgets dialog instead of silently creating an empty map.
	wxArrayString TemplateChoices;
	TArray<FString> TemplatePackageNames;
	INT DefaultSelection = 0;

	for (INT TemplateIndex = 0; TemplateIndex < Templates.Num(); ++TemplateIndex)
	{
		UTemplateMapMetadata* Metadata = Templates(TemplateIndex);
		if (Metadata != NULL)
		{
			const FString PackageName = Metadata->GetName();
			const FString DisplayName = Localize(
				TEXT("TemplateMapMetadata.DisplayNames"),
				*PackageName,
				TEXT("EditorMapTemplates"),
				UObject::GetLanguage(),
				FALSE);

			TemplateChoices.Add(wxString(*DisplayName));
			TemplatePackageNames.AddItem(PackageName);
			if (PackageName == TEXT("Template_MidDay"))
			{
				DefaultSelection = TemplatePackageNames.Num() - 1;
			}
		}
	}

	// Match the original New Map screen by retaining an explicit truly-empty
	// level option after the supplied lighting templates.
	TemplateChoices.Add(wxString(TEXT("Blank Map")));
	TemplatePackageNames.AddItem(TEXT(""));

	wxSingleChoiceDialog NewMapDialog(
		GApp->EditorFrame,
		wxString(TEXT("Choose a starter lighting template or a blank level.")),
		wxString(*LocalizeUnrealEd("NewMapScreen_Title")),
		TemplateChoices);
	NewMapDialog.SetSelection(DefaultSelection);

	if (NewMapDialog.ShowModal() == wxID_OK)
	{
		const INT Selection = NewMapDialog.GetSelection();
		if (Selection >= 0 && Selection < TemplatePackageNames.Num())
		{
			TemplateName = TemplatePackageNames(Selection);
			bCreateMap = TRUE;
		}
	}
#endif

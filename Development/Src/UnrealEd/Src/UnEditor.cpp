...
:	bHandlersInitialized( FALSE )
{
}

/**
 * Loads the built-in handlers only after the singleton itself is fully
 * constructed. Handler construction registers back through Instance(), so
 * doing this work in FReimportManager's constructor deadlocks with modern
 * thread-safe function-local static initialization.
 */
void FReimportManager::InitializeHandlers()
{
	...
	// Set this before loading classes because their handlers register through
	// Instance() and therefore re-enter this function.

...
void AUTBot::UpdateEnemyInfo(APawn* AcquiredEnemy)
{
	if( AcquiredEnemy )
	{
		...
		LastSeeingPos = ViewTarget ? ViewTarget->Location : Location;
		LastSeenPos = AcquiredEnemy->Location;
...
void AUTBot::execCanMakePathTo( FFrame& Stack, RESULT_DECL )
{
	...
	ANavigationPoint* CurrentPathEnd = CurrentPath ? CurrentPath->End.Nav() : NULL;
	if ( N && Pawn && Pawn->CylinderComponent && Pawn->ValidAnchor() && CurrentPathEnd
		&& ((CurrentPath->reachFlags & R_WALK) == CurrentPath->reachFlags) )
	{
		UReachSpec *NextPath = CurrentPathEnd->GetReachSpecTo(N);
			...
			&& NextPath->supports(Pawn->CylinderComponent->CollisionRadius,Pawn->CylinderComponent->CollisionHeight,Pawn->calcMoveFlags(),Pawn->MaxFallSpeed) )
		{
			...
			if ( !Hit.Actor )
			{
				...
				FVector LineDist = CurrentPathEnd->Location - (Pawn->Location + (Dir | (CurrentPathEnd->Location - Pawn->Location)) * Dir);

...
function DrawScore()
{
	...
	if (bShowTempScoreboard)
	{
		...
		{
			Canvas.SetPos(Left, Top);
			if ( WorldInfo.GRI.PRIArray[i] == PlayerOwner.PlayerReplicationInfo )
				Canvas.DrawColor.B = 0;
			else
				Canvas.DrawColor = WhiteColor;
			Canvas.DrawText(WorldInfo.GRI.PRIArray[i].PlayerName$": "$int(WorldInfo.GRI.PRIArray[i].Score));
			Top += yl;
		}
	}
}
...
defaultproperties
{
	XboxA_Tex=Texture2D'UDKFrontEnd.xbox_a_png'
	HudTexture=Texture2D'UI_HUD.HUD.UI_HUD_BaseA'

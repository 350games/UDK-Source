...
// FIXMESTEVE - only if no valid character	if ( PawnSetupRecord.Species == None )
	...
	{
		UTPlayerReplicationInfo(PlayerReplicationInfo).bIsFemale = true;
	}
}
...
reliable server function ServerPlayerPreferences( UTPawn.EWeaponHand NewWeaponHand, bool bNewAutoTaunt)
{
...
reliable client function ClientPlayAnnouncement(
...
state BaseSpectating
{
    ...
    function PlayerMove(float DeltaTime)
    {
		if ( (Pawn(ViewTarget) != None) && (WorldInfo.NetMode == NM_Client) )
		{
...
/** Returns whether this player wants the landing camera shake. */
...
{
	if ( WorldInfo.NetMode == NM_StandAlone && bViewingMatineeCinematic )
	{
		WorldInfo.Game.SceneAbort();
	}
...
{
	if ( VoiceChangeLimit > WorldInfo.TimeSeconds )
		return;

	VoiceChangeLimit = WorldInfo.TimeSeconds + 10.0;	// TODO - probably better to hook this up to the same limit system that playernames use
...
{
	if ( WorldInfo.NetMode == NM_Standalone || Vehicle(Pawn) != None || PlayerReplicationInfo.bOnlySpectator || PlayerReplicationInfo.bAdmin || IsA('Admin') )
	{
...
event PlayerCalcView(out actor ViewActor, out vector CameraLocation, out rotator CameraRotation )
{
	...
	if ( LastPlayerCalcView == WorldInfo.TimeSeconds && CalcViewActor != None && CalcViewActor.Location == CalcViewActorLocation )
	{
...
function BecomeSpectator()
{
	...
	if ( !WorldInfo.Game.BecomeSpectator(self) )
		return;
	...
	ServerSpectate();
	BroadcastLocalizedMessage(WorldInfo.Game.GameMessageClass, 14, PlayerReplicationInfo);
...
function BecomeActivePlayer()
{
	...
	if ( !WorldInfo.Game.AllowBecomeActivePlayer(self) )
		return;
	...
	BroadcastLocalizedMessage(WorldInfo.Game.GameMessageClass, 1, PlayerReplicationInfo);
	if (WorldInfo.Game.bTeamGame)
		WorldInfo.Game.ChangeTeam(self, WorldInfo.Game.PickTeam(int(GetURLOption("Team")), None), false);
	if (!WorldInfo.Game.bDelayedStart)
    {
		// start match, or let player enter, immediately
		WorldInfo.Game.bRestartLevel = false;  // let player spawn once in levels that must be restarted after every death
		if (WorldInfo.Game.bWaitingToStartMatch)
			WorldInfo.Game.StartMatch();
		else
			WorldInfo.Game.RestartPlayer(PlayerController(Owner));
		WorldInfo.Game.bRestartLevel = WorldInfo.Game.Default.bRestartLevel;
    }
    else
...
function ServerViewNextPlayer()
{
    ...
    {
        if ( WorldInfo.Game.CanSpectate(self, C.PlayerReplicationInfo) )
        {
            if ( Pick == None )
                Pick = C;
            if ( bFound )
...
function SendVoiceMessage(PlayerReplicationInfo Sender, PlayerReplicationInfo Recipient, name messagetype, byte messageID, name broadcasttype)
{
	...
	foreach WorldInfo.AllControllers(class'Controller', P)
	{
		if ( PlayerController(P) != None )
		{
			...
			{
				if ( (broadcasttype == 'GLOBAL') || !WorldInfo.Game.bTeamGame || (Sender.Team == P.PlayerReplicationInfo.Team) )

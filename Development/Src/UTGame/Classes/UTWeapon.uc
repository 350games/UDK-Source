...
simulated function DisplayDebug(HUD HUD, out float out_YL, out float out_YPos)
{
	...
	T = "Eyeheight "$Instigator.EyeHeight$" base "$Instigator.BaseEyeheight;
...
simulated function ActiveRenderOverlays( HUD H )
{
	...
	if (WeaponFade < WorldInfo.TimeSeconds)
		return;

	if ( WeaponFade - WorldInfo.TimeSeconds < (WeaponFadeTime/2) )
		H.Canvas.DrawColor.A = 255 * ( (WeaponFade-WorldInfo.TimeSeconds) / (WeaponFadeTime/2));
...
simulated function PlayWeaponAnimation(Name Sequence, float fDesiredDuration, optional bool bLoop, optional SkeletalMeshComponent SkelMesh)
{
	...
	// Play Animation
	AnimNodeSequence(SkelMesh.Animations).PlayAnim(bLoop, fNewRate);
}
...
simulated event CauseMuzzleFlashLight()
{
	...
	MuzzleFlashLight.SetLightProperties(MuzzleFlashLightBrightness);
 ...
 */
simulated function AttachWeaponTo( SkeletalMeshComponent MeshCpnt, optional Name BoneName )
{
...
reliable server function ServerWeaponStartFire(byte FireModeNum)
{
...
reliable server function ServerWeaponStopFire( byte FireModeNum )
{
	WeaponEndFire(FireModeNum);
}
...
simulated function ProcessInstantHit(byte FiringMode, ImpactInfo Impact, optional int NumHits)
{
	if ( Impact.HitActor != None )
	{
		...
		Impact.HitActor.TakeDamage(InstantHitDamage[FiringMode] * NumHits, Instigator.Controller,
			Impact.HitLocation, Impact.RayDir, InstantHitDamageTypes[FiringMode], Impact.HitInfo, self);
	}
}
...
simulated function IncrementFlashCount()
{
	...
	if ( UTPawn(Instigator) != none )
		UTPawn(Instigator).IncrementFlashCount(self, CurrentFireMode);
}
...
function SetFlashLocation(vector HitLocation)
{
	...
	if ( UTPawn(Instigator)!=None )
		UTPawn(Instigator).SetFlashLocation(self, CurrentFireMode, HitLocation);
}
...
simulated state Active
{
	...
	simulated event BeginState(Name PreviousStateName)
	{
		...
		Super.BeginState(PreviousStateName);	// Play the Idle Animation
...
simulated state WeaponEquipping
{
	...
	simulated event BeginState(Name PreviousStateName)
	{
		...
		if (Instigator.IsLocallyControlled() && UTPawn(Instigator)!=None)
		{
			...
			WeaponFade = WorldInfo.TimeSeconds + WeaponFadeTime;
		}
	}

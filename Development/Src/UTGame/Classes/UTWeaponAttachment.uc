...
function AttachTo( SkeletalMeshComponent MeshCpnt, name BoneName )
{
	...
	{
		Mesh.SetOwnerNoSee(!UTPawn(Instigator).bBehindView);
	}
...
simulated event ThirdPersonFireEffects()
{
	...
	if ( UTCharacter(Instigator) != None )
	{
		// FIXME: Come up with a better way to support the animspeed.

		if ( UTPawn(Instigator).FiringMode == 1 )
			UTCharacter(Instigator).PlayFiring(1.0,'1');
		else
			UTCharacter(Instigator).PlayFiring(1.0,'0');
	}
}
...
simulated function CheckForSplash()
{
	...
	if ( !WorldInfo.bDropDetail && (WorldInfo.GetDetailMode() != DM_Low) && (SplashEffect != None) && !Instigator.PhysicsVolume.bWaterVolume )
	{
		// check for splash

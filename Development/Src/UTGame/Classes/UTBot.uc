...
native noexport final latent function WaitToSeeEnemy(); // return when looking directly at visible enemy
native noexport final function bool CanMakePathTo(Actor A); // assumes valid CurrentPath, tries to see if CurrentPath can be combine with path to N
...
{
	if ( (InstigatedBy != None) && (InstigatedBy.Pawn != Pawn) )
		DamageAttitudeTo(InstigatedBy.Pawn, Damage);
}

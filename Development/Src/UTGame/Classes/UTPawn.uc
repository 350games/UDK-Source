...
replication
{
    ...
    if (Role==ROLE_Authority)
        ShieldStrength, CurrentWeaponAttachmentClass;
}
...
//			UTWeapon(Weapon).WeaponAttachment.Mesh.bOwnerNoSee = !bVisible;
			CurrentWeaponAttachment.Mesh.SetOwnerNoSee(!bVisible);
		}
	}
...
State Dying
{
ignores OnAnimEnd, Bump, HitWall, HeadVolumeChange, PhysicsVolumeChange, Falling, BreathTimer, Landed;
...
simulated function SetFlashLocation(Weapon InWeapon, byte InFiringMode, vector NewLoc)
{
	...
	SetTimer(0.10, false, 'ClearFlashLocationTimer');

	// This weapon has fired.

	WeaponFired(InWeapon, false, NewLoc);
}
...
function ClearFlashLocationTimer()
{
	ClearFlashLocation(Weapon);
}

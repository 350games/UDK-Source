...
void FD3D9DynamicRHI::SetVertexShaderFloatArray(FVertexShaderRHIParamRef VertexShaderRHI,UINT /*BufferIndex*/,UINT BaseIndex,UINT NumValues,const FLOAT* FloatValues, INT /*ParamIndex*/)
{
	...
	FLOAT pPaddedVectorValues[GroupSize][4];
	...
	while( NumValues )
	{
		...
		// The snapshot lacks the XNAMath SDK header.  Preserve the required
		// float-to-float4 padding directly; this path is not performance-critical.
		for( UINT ValueIndex = 0; ValueIndex < NumValuesThisGroup; ValueIndex++ )
		{
			pPaddedVectorValues[ValueIndex][0] = FloatValues[ValueIndex];
			pPaddedVectorValues[ValueIndex][1] = 0.0f;
			pPaddedVectorValues[ValueIndex][2] = 0.0f;
			pPaddedVectorValues[ValueIndex][3] = 0.0f;
		}

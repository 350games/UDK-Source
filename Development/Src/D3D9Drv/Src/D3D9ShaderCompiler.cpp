...
#if !UE3_LEAN_AND_MEAN || WITH_LOCAL_UTGAME_D3D9_SHADER_COMPILER

#if WITH_LOCAL_UTGAME_D3D9_SHADER_COMPILER
// The recovered lean target compiles its first-run SM3 cache through the
// Windows SDK compiler.  It still includes D3DX headers for the renderer's
// retained types, but does not link against a D3DX runtime.
#include "D3Dcompiler.h"
typedef ID3DInclude FD3DIncludeInterface;
typedef D3D_INCLUDE_TYPE FD3DIncludeType;
#else
typedef ID3DXInclude FD3DIncludeInterface;
typedef D3DXINCLUDE_TYPE FD3DIncludeType;
#endif

/**
 * An implementation of the active shader compiler include interface to access a FShaderCompilerEnvironment.
 */
class FD3DIncludeEnvironment : public FD3DIncludeInterface
{
public:

	STDMETHOD(Open)(FD3DIncludeType Type,LPCSTR Name,LPCVOID ParentData,LPCVOID* Data,UINT* Bytes)
	{
...
/**
 * Preprocesses a shader (resolve all #includes and #defines) and dumps it out for debugging.
 */
	...
	)
{
	...
	// D3DXMACRO and D3D_SHADER_MACRO are the same two-pointer ABI.  Keeping
	// the former in the surrounding legacy code avoids changing its worker
	// protocol while the local path calls the Windows SDK compiler.
	HRESULT PreProcessHR = D3DPreprocess(
		(ANSICHAR*)AnsiSourceFile,
		AnsiSourceFile.Length(),
		TCHAR_TO_ANSI(SourceFilename),
		reinterpret_cast<const D3D_SHADER_MACRO*>(&Macros(0)),
		&IncludeEnvironment,
	...
	{
		#if WITH_LOCAL_UTGAME_D3D9_SHADER_COMPILER
		// D3DPreprocess is allowed to omit diagnostics on an early failure.
		// Do not let a failed debug dump turn into a null-dereference.
		const TCHAR* ErrorText = PreProcessErrors ? ANSI_TO_TCHAR((ANSICHAR*)PreProcessErrors->GetBufferPointer()) : TEXT("No compiler diagnostics were returned.");
		warnf( NAME_Warning, TEXT("Preprocess failed for shader %s: %s"), SourceFilename, ErrorText );
		#else
...
/** Information about a constant passed back from the shader compiler. */
...
/**
 * D3DCompile emits valid Shader Model 3 bytecode, but D3DReflect does not
 * support that legacy bytecode format. D3DDisassemble preserves the D3D9
 * register table, which contains the same top-level parameter information
 * the original path copied from ID3DXConstantTable.
 */
static void D3D9ExtractConstantsFromDisassembly(const ANSICHAR* Disassembly, TArray<FD3D9ConstantDesc>& Constants)
{
	const ANSICHAR* RegisterTable = strstr(Disassembly, "//   ------------");
	if (RegisterTable == NULL)
	{
		return;
	}

	const ANSICHAR* Cursor = strchr(RegisterTable, '\n');
	while (Cursor != NULL)
	{
		++Cursor;
		...
		const ANSICHAR* LineEnd = strchr(Cursor, '\n');
		...
		appMemcpy(Line, Cursor, CopyLength);
		Line[CopyLength] = 0;

		ANSICHAR Name[FD3D9ConstantDesc::MaxNameLength];
		ANSICHAR Register[32];
		...
		{
			const ANSICHAR* RegisterIndexText = Register + 1;
			if ((Register[0] == 'c' || Register[0] == 's' || Register[0] == 'i' || Register[0] == 'b') && *RegisterIndexText >= '0' && *RegisterIndexText <= '9')
			{
				UINT RegisterIndex = 0;
				for (const ANSICHAR* Digit = RegisterIndexText; *Digit >= '0' && *Digit <= '9'; ++Digit)
				{
					RegisterIndex = RegisterIndex * 10 + (*Digit - '0');
				}
...
/**
 * Keep SEH out of the RAII-owning compile routine below. Modern MSVC rejects
 * __try in a function that needs C++ object unwinding (/EHsc), so this helper
 * deliberately uses only plain pointer parameters and locals.
 */
static HRESULT D3D9SafeCompileShader(
	LPCSTR Source,
	SIZE_T SourceLength,
	...
	ID3DInclude* IncludeEnvironment,
	LPCSTR FunctionName,
	LPCSTR ShaderProfile,
	UINT CompileFlags,
	ID3DBlob** Shader,
	ID3DBlob** Errors,
	UBOOL* bCaughtException
	)
{
...
/** See D3D9SafeCompileShader for why this is a plain-pointer SEH wrapper. */
static HRESULT D3D9SafeDisassembleShader(
	LPCVOID ShaderByteCode,
...
/** Compiles a D3D9 shader through the Windows SDK shader compiler. */
// Not static as currently OpenGL RHI also uses this function
/*static*/ UBOOL D3D9CompileShaderThroughDll(
	const TCHAR* SourceFilename,
	const TCHAR* FunctionName,
	...
	)
{
	...
	// D3DX's D3D9 compiler flags mirror D3DCompile's flags.  D3DCompile
	// supports attributes directly; retain the established macro value and
	// omit the obsolete D3DX 9.31 fallback bit if it is ever present.
	(void)AttributeMacroIndex;
	UBOOL bCompileCaughtException = FALSE;
	HRESULT Result = D3D9SafeCompileShader(
		(ANSICHAR*)AnsiSourceFile,
		AnsiSourceFile.Length(),
		TCHAR_TO_ANSI(SourceFilename),
		...
		TCHAR_TO_ANSI(FunctionName),
		TCHAR_TO_ANSI(ShaderProfile),
		CompileFlags & ~D3DCOMPILE_RESERVED16,
		Shader.GetInitReference(),
		Errors.GetInitReference(),
		&bCompileCaughtException);

	if (bCompileCaughtException)
	{
		warnf(NAME_Warning, TEXT("D3DCompile threw an exception while compiling %s/%s for profile %s"), SourceFilename, FunctionName, ShaderProfile);
		return FALSE;
	}
	...
	const UINT NumShaderBytes = (UINT)Shader->GetBufferSize();
	Output.Code.Empty(NumShaderBytes);
	Output.Code.Add(NumShaderBytes);
	appMemcpy(&Output.Code(0), Shader->GetBufferPointer(), NumShaderBytes);

	TRefCountPtr<ID3DBlob> DisassemblyBuffer;
	UBOOL bDisassembleCaughtException = FALSE;
	HRESULT DisassembleResult = D3D9SafeDisassembleShader(
		Shader->GetBufferPointer(),
		Shader->GetBufferSize(),
		0,
		NULL,
		DisassemblyBuffer.GetInitReference(),
		&bDisassembleCaughtException);

	if (bDisassembleCaughtException)
	{
		warnf(NAME_Warning, TEXT("D3DDisassemble threw an exception while compiling %s/%s"), SourceFilename, FunctionName);
		return FALSE;
	}

	if (FAILED(DisassembleResult))
	{
		FilteredErrors.AddItem(TEXT("D3DDisassemble failed after shader compilation."));
		return FALSE;
	}

	D3D9ExtractConstantsFromDisassembly((const ANSICHAR*)DisassemblyBuffer->GetBufferPointer(), Constants);
	DisassemblyString = ANSI_TO_TCHAR((ANSICHAR*)DisassemblyBuffer->GetBufferPointer());
	return TRUE;
}
...
static void ValidateD3D9WorkerOutputRange(
...
{
	checkf(
		CurrentPosition >= 0 && CurrentPosition <= WorkerOutput.Num(),
		TEXT("Invalid D3D9 shader-worker offset while reading %s: thread=%u batch=%d job=%d offset=%d outputBytes=%d"),
		FieldName,
		ThreadId,
		BatchIndex,
		JobId,
	...
	const UINT RemainingBytes = (UINT)(WorkerOutput.Num() - CurrentPosition);
	checkf(
		Size <= RemainingBytes,
		TEXT("Truncated D3D9 shader-worker output while reading %s: thread=%u batch=%d job=%d offset=%d requestedBytes=%u remainingBytes=%u outputBytes=%d"),
		FieldName,
		ThreadId,
		BatchIndex,
		JobId,
...
template<class T>
static void ReadD3D9WorkerOutputValueChecked(
	T& Value,
	INT& CurrentPosition,
	const TArray<BYTE>& WorkerOutput,
	const TCHAR* FieldName,
	...
	UINT ThreadId)
{
	ValidateD3D9WorkerOutputRange(WorkerOutput, CurrentPosition, (UINT)sizeof(T), FieldName, BatchIndex, JobId, ThreadId);
	WorkerOutputReadValue(Value, CurrentPosition, WorkerOutput);
}

static void ReadD3D9WorkerOutputMemoryChecked(
	void* Dest,
	UINT Size,
	INT& CurrentPosition,
	const TArray<BYTE>& WorkerOutput,
	...
	UINT ThreadId)
{
	ValidateD3D9WorkerOutputRange(WorkerOutput, CurrentPosition, Size, FieldName, BatchIndex, JobId, ThreadId);
	WorkerOutputReadMemory(Dest, Size, CurrentPosition, WorkerOutput);
}
	...
	UINT ThreadId)
{
	...
	BYTE ReadVersion;
	ReadD3D9WorkerOutputValueChecked(ReadVersion, CurrentPosition, WorkerOutput, TEXT("D3D9 output version"), BatchIndex, JobId, ThreadId);
	checkf(
		ReadVersion == D3D9ShaderCompileWorkerOutputVersion,
		TEXT("Unsupported D3D9 shader-worker output version: thread=%u batch=%d job=%d version=%u expected=%u"),
		ThreadId,
		BatchIndex,
		JobId,
		ReadVersion,
		D3D9ShaderCompileWorkerOutputVersion);
	EWorkerJobType OutJobType;
	ReadD3D9WorkerOutputValueChecked(OutJobType, CurrentPosition, WorkerOutput, TEXT("D3D9 job type"), BatchIndex, JobId, ThreadId);
	checkf(
		OutJobType == WJT_D3D9Shader,
		TEXT("Unexpected D3D9 shader-worker job type: thread=%u batch=%d job=%d type=%d"),
		ThreadId,
		BatchIndex,
		JobId,
		OutJobType);
	HRESULT CompileResult;
	ReadD3D9WorkerOutputValueChecked(CompileResult, CurrentPosition, WorkerOutput, TEXT("D3D9 compile result"), BatchIndex, JobId, ThreadId);
	UINT ByteCodeLength;
	ReadD3D9WorkerOutputValueChecked(ByteCodeLength, CurrentPosition, WorkerOutput, TEXT("D3D9 bytecode length"), BatchIndex, JobId, ThreadId);
	ValidateD3D9WorkerOutputRange(WorkerOutput, CurrentPosition, ByteCodeLength, TEXT("D3D9 bytecode"), BatchIndex, JobId, ThreadId);
	Output.Code.Empty(ByteCodeLength);
	Output.Code.Add(ByteCodeLength);
	if (ByteCodeLength > 0)
	{
		ReadD3D9WorkerOutputMemoryChecked(&Output.Code(0), ByteCodeLength, CurrentPosition, WorkerOutput, TEXT("D3D9 bytecode"), BatchIndex, JobId, ThreadId);
	}
	UINT ErrorStringLength;
	ReadD3D9WorkerOutputValueChecked(ErrorStringLength, CurrentPosition, WorkerOutput, TEXT("D3D9 error string length"), BatchIndex, JobId, ThreadId);
	ValidateD3D9WorkerOutputRange(WorkerOutput, CurrentPosition, ErrorStringLength, TEXT("D3D9 error string"), BatchIndex, JobId, ThreadId);
	
	FString ErrorString;
	if (ErrorStringLength > 0)
	{
		ANSICHAR* ErrorBuffer = new ANSICHAR[ErrorStringLength + 1];
		ReadD3D9WorkerOutputMemoryChecked(ErrorBuffer, ErrorStringLength, CurrentPosition, WorkerOutput, TEXT("D3D9 error string"), BatchIndex, JobId, ThreadId);
	...
	ReadD3D9WorkerOutputValueChecked(ConstantArrayLength, CurrentPosition, WorkerOutput, TEXT("D3D9 constant array length"), BatchIndex, JobId, ThreadId);
	checkf(
		ConstantArrayLength % sizeof(FD3D9ConstantDesc) == 0,
		TEXT("Misaligned D3D9 constant array: thread=%u batch=%d job=%d bytes=%u descriptorBytes=%u offset=%d outputBytes=%d"),
		ThreadId,
		BatchIndex,
		JobId,
		ConstantArrayLength,
		(UINT)sizeof(FD3D9ConstantDesc),
		CurrentPosition,
		WorkerOutput.Num());
	ValidateD3D9WorkerOutputRange(WorkerOutput, CurrentPosition, ConstantArrayLength, TEXT("D3D9 constant array"), BatchIndex, JobId, ThreadId);
	...
	if(ConstantArrayLength > 0)
	{
		...
		ReadD3D9WorkerOutputMemoryChecked(&Constants(0), ConstantArrayLength, CurrentPosition, WorkerOutput, TEXT("D3D9 constant array"), BatchIndex, JobId, ThreadId);
	}

	UINT DisassemblyStringLength;
	ReadD3D9WorkerOutputValueChecked(DisassemblyStringLength, CurrentPosition, WorkerOutput, TEXT("D3D9 disassembly length"), BatchIndex, JobId, ThreadId);
	ValidateD3D9WorkerOutputRange(WorkerOutput, CurrentPosition, DisassemblyStringLength, TEXT("D3D9 disassembly"), BatchIndex, JobId, ThreadId);

	FString DisassemblyString;
	if (DisassemblyStringLength > 0)
	{
		ANSICHAR* DisassemblyStringAnsi = new ANSICHAR[DisassemblyStringLength + 1];
		ReadD3D9WorkerOutputMemoryChecked(DisassemblyStringAnsi, DisassemblyStringLength, CurrentPosition, WorkerOutput, TEXT("D3D9 disassembly"), BatchIndex, JobId, ThreadId);

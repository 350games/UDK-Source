...
#define IMPLEMENT_PLATFORM_INTERFACE_SINGLETON(Class, ClassDesc) \
 ...
 \
			/* Empty class names intentionally select the fallback/default implementation. */ \
			/* Do not pass an empty name to LoadClass, which reports a misleading 'Class None.' warning. */ \
			UClass* SingletonClass = NULL; \
			if (SingletonClassName.Len() > 0) \
			{ \

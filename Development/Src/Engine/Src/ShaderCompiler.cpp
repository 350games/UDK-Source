...
void WorkerOutputReadMemory(void* Dest, UINT Size, INT& CurrentPosition, const TArray<BYTE>& Buffer)
{
	...
	check(CurrentPosition <= Buffer.Num());
	check(Size <= (UINT)(Buffer.Num() - CurrentPosition));
	if (Size > 0)
	{
...
/**
 * Validates a range in a shader-worker output without performing signed
 * addition. Worker-provided sizes must never be allowed to wrap an INT before
 * an allocation or memcpy.
 */
static void ValidateWorkerOutputRange(
	const TArray<BYTE>& WorkerOutput,
	INT CurrentPosition,
	UINT Size,
	...
	UINT ThreadId)
{
	checkf(
		...
		TEXT("Invalid shader-worker offset while reading %s: thread=%u batch=%d job=%d offset=%d outputBytes=%d"),
		FieldName,
		ThreadId,
		BatchIndex,
		JobId,
	...
	checkf(
		Size <= RemainingBytes,
		TEXT("Truncated shader-worker output while reading %s: thread=%u batch=%d job=%d offset=%d requestedBytes=%u remainingBytes=%u outputBytes=%d"),
		FieldName,
		ThreadId,
		BatchIndex,
		JobId,
...
template<class T>
static void ReadWorkerOutputValueChecked(
	T& Value,
	INT& CurrentPosition,
	const TArray<BYTE>& WorkerOutput,
	const TCHAR* FieldName,
	...
	UINT ThreadId)
{
	ValidateWorkerOutputRange(WorkerOutput, CurrentPosition, (UINT)sizeof(T), FieldName, BatchIndex, JobId, ThreadId);
	WorkerOutputReadValue(Value, CurrentPosition, WorkerOutput);
}

static void ReadWorkerOutputMemoryChecked(
	void* Dest,
	UINT Size,
	INT& CurrentPosition,
	const TArray<BYTE>& WorkerOutput,
	...
	UINT ThreadId)
{
	ValidateWorkerOutputRange(WorkerOutput, CurrentPosition, Size, FieldName, BatchIndex, JobId, ThreadId);
...
void FShaderCompilingThreadManager::BeginCompilingThreadLoop(UINT CurrentThreadId)
{
	...
	while (!bIsDone)
	{
		...
		if (JobIndex < CompileQueue.Num())
		{
...
#if PLATFORM_DESKTOP && !USE_NULL_RHI && (!UE3_LEAN_AND_MEAN || WITH_LOCAL_UTGAME_D3D9_SHADER_COMPILER) && !CONSOLE
			...
			// This recovered lean target deliberately supports just its retained D3D9
			// renderer.  Do not pull the D3D11/OpenGL compiler modules back in with
			// the D3D9 first-run shader-cache path.
...
void FShaderCompilingThreadManager::FinishWorkerCompile(INT BatchedJobId, UINT ThreadId)
{
	...
	ReadWorkerOutputValueChecked(ShaderCompileWorkerOutputVersion, CurrentPosition, WorkerOutput, TEXT("worker output version"), INDEX_NONE, INDEX_NONE, ThreadId);
	checkf(
		ShaderCompileWorkerOutputVersion == 0,
		TEXT("Unsupported shader-worker output version: thread=%u version=%d outputBytes=%d"),
		ThreadId,
		ShaderCompileWorkerOutputVersion,
		WorkerOutput.Num());

	INT NumBatches;
	ReadWorkerOutputValueChecked(NumBatches, CurrentPosition, WorkerOutput, TEXT("batch count"), INDEX_NONE, INDEX_NONE, ThreadId);
	const INT MinimumBatchBytes = (INT)(sizeof(INT) + sizeof(BYTE) + sizeof(EWorkerJobType));
	const INT RemainingBatchBytes = WorkerOutput.Num() - CurrentPosition;
	checkf(
		NumBatches >= 0 && NumBatches <= RemainingBatchBytes / MinimumBatchBytes,
		TEXT("Invalid shader-worker batch count: thread=%u batches=%d offset=%d remainingBytes=%d outputBytes=%d"),
		ThreadId,
		NumBatches,
		CurrentPosition,
		RemainingBatchBytes,
	...
	{
		INT JobId;
		ReadWorkerOutputValueChecked(JobId, CurrentPosition, WorkerOutput, TEXT("job id"), BatchIndex, INDEX_NONE, ThreadId);

		INT BatchBeginPosition = CurrentPosition;
		const BYTE WorkerErrorOutputVersion = 0;
		BYTE ReadVersion;
		ReadWorkerOutputValueChecked(ReadVersion, BatchBeginPosition, WorkerOutput, TEXT("job output version"), BatchIndex, JobId, ThreadId);
		EWorkerJobType OutJobType;
		ReadWorkerOutputValueChecked(OutJobType, BatchBeginPosition, WorkerOutput, TEXT("job type"), BatchIndex, JobId, ThreadId);
		if (OutJobType == WJT_WorkerError)
		{
			// The worker terminated with an error, read it out of the file and assert on it.
			checkf(
				ReadVersion == WorkerErrorOutputVersion,
				TEXT("Unsupported shader-worker error version: thread=%u batch=%d job=%d version=%u"),
				ThreadId,
				BatchIndex,
				JobId,
				ReadVersion);
			UINT ErrorStringSize;
			ReadWorkerOutputValueChecked(ErrorStringSize, BatchBeginPosition, WorkerOutput, TEXT("worker error string length"), BatchIndex, JobId, ThreadId);
			checkf(
				ErrorStringSize % sizeof(TCHAR) == 0,
				TEXT("Misaligned shader-worker error string: thread=%u batch=%d job=%d bytes=%u TCHARBytes=%u"),
				ThreadId,
				BatchIndex,
				JobId,
				ErrorStringSize,
				(UINT)sizeof(TCHAR));
			ValidateWorkerOutputRange(WorkerOutput, BatchBeginPosition, ErrorStringSize, TEXT("worker error string"), BatchIndex, JobId, ThreadId);
			TCHAR* ErrorBuffer = new TCHAR[ErrorStringSize / sizeof(TCHAR) + 1];
			ReadWorkerOutputMemoryChecked(ErrorBuffer, ErrorStringSize, BatchBeginPosition, WorkerOutput, TEXT("worker error string"), BatchIndex, JobId, ThreadId);
			ErrorBuffer[ErrorStringSize / sizeof(TCHAR)] = 0;
			const FString WorkerErrorString(ErrorBuffer);
			delete [] ErrorBuffer;
			appErrorf(TEXT("%s for thread %u batch %d job %d terminated with message: \n %s"), *ShaderCompileWorkerName, ThreadId, BatchIndex, JobId, *WorkerErrorString);
		}

		// This check can fail under very rare conditions. We haven't tracked that down yet.
		// Initiating a shader recompile should help
		checkf(
			CompileQueue.IsValidIndex(JobId),
			TEXT("Invalid shader-worker job id: thread=%u batch=%d job=%d compileQueueJobs=%d offset=%d outputBytes=%d"),
			ThreadId,
			BatchIndex,
			JobId,
			CompileQueue.Num(),
		...
		checkf(
			!CurrentJob->bFinalized,
			TEXT("Duplicate shader-worker result: thread=%u batch=%d job=%d offset=%d outputBytes=%d"),
			ThreadId,
			BatchIndex,
			JobId,
			CurrentPosition,
...
#if _WINDOWS && !USE_NULL_RHI && (!UE3_LEAN_AND_MEAN || WITH_LOCAL_UTGAME_D3D9_SHADER_COMPILER) && !CONSOLE
	...
	checkf(
		CurrentPosition == WorkerOutput.Num(),
		TEXT("Unexpected trailing shader-worker output: thread=%u batches=%d parsedBytes=%d outputBytes=%d trailingBytes=%d"),
		ThreadId,
		NumBatches,
		CurrentPosition,
		WorkerOutput.Num(),
		WorkerOutput.Num() - CurrentPosition);
}

...
#if WITH_MODERN_LIBPNG

// The recovered local target compiles the bundled libPNG sources with its
// current C runtime instead of linking the pre-UCRT binary archive.

#elif PNG_LIBPNG_VER >= 10513
...
UBOOL FPNGLoader::Decode( BYTE* OutDecompressedData )
{
	...
	PNGReadGuard PNGGuard( &PNGReadStruct, &PNGInfo );
	{
		...
		{
			// The bundled libpng is 1.2.x, where this transform is named
			// png_set_filler rather than the later png_set_add_alpha alias.
			png_set_filler(PNGReadStruct, 255, PNG_FILLER_AFTER);
		}

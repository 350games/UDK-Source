...
#endif
		// Setup bounds for version info text 1.  The editor also displays a
		// startup-progress line in the lower-left corner, so give the version
		// line its own row rather than drawing both strings at the same position.
		if( GIsEditor )
		{
			GSplashScreenTextRects[ SplashTextType::VersionInfo1 ].top = bm.bmHeight - 44;
			GSplashScreenTextRects[ SplashTextType::VersionInfo1 ].bottom = bm.bmHeight - 24;
		}
		else
		{

...
#if (!CONSOLE && !PLATFORM_MACOSX && WITH_NVIDIA_STEREO)
...
#else /* no NVAPI-backed stereo support */
namespace nv {
	namespace stereo {
		...
		#if _WINDOWS && !NO_STEREO_D3D9
		struct UE3StereoD3D9
		{
			struct Parms
			{
				static const int StereoTexWidth = 8;
			...
			UE3StereoD3D9(bool) {}
			bool RequiresUpdate(bool) { return false; }
			void UpdateStereoTexture(IDirect3DDevice9*, IDirect3DTexture9*, bool) {}
		};
		#endif

		#if _WINDOWS && !NO_STEREO_D3D11
		struct UE3StereoD3D11
		{
			struct Parms
			{
				static const int StereoTexWidth = 8;
			...
			UE3StereoD3D11(bool) {}
			bool RequiresUpdate(bool) { return false; }
			void UpdateStereoTexture(ID3D11Device*, ID3D11Texture2D*, bool) {}
		};
		#endif
	};
};
